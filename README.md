...
Python 3.11+, PostgreSQL 14+/SQLite, asyncpg 0.30.0+/aiosqlite 0.21.0+, aiogram 3.22+
...
Если у вас нет готовой бд, этот скрипт ни на что не повлияет, он создаёт бд в зависимости от переменной DB_TYPE, которую вы указали в предыдущем шаге. 
``` bash
python -m bot.database.init_db
```
Новая база данных создастся в каталоге проекта под именем todo_db, если вы используете sql, или на localhost:5432, если используете postgres.
### 6. Запустите бота локально
Все таблицы создадуться сами, если их не было.
```
python -m main
```
Готово✅
